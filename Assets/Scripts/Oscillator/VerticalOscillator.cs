...
{
    [System.Serializable]
    public struct SpringOscillator
    {
    ...
    public List<SpringOscillator> oscillators = new List<SpringOscillator>();
    ...
    private void FixedUpdate()
    {
        ...
        {
            SpringOscillator currentOscillator = oscillators[i];
            ...
            float acceleration = -((currentOscillator.springConstant / currentOscillator.mass) * currentDisplacementFromEquilibrium) + currentOscillator.gravity;
    ...
    private void SetupOscillators()
    {
        if (formChain)
        {
            ...
            {
                SpringOscillator newOscillator = new SpringOscillator();
        ...
        for (int i = 0; i < oscillators.Count; i++)
        {
            SpringOscillator current = oscillators[i];
            ...
            //Setup Line Renderer:
            current.lineRenderer = massObject.GetComponent<LineRenderer>();
            if (current.lineRenderer == null)
            {
                current.lineRenderer = massObject.AddComponent<LineRenderer>();
            }
            SetupLineRenderer(current.lineRenderer);
    ...
    private void UpdateSpringVisual(SpringOscillator oscillator)
    {
        LineRenderer line = oscillator.lineRenderer;
        ...
        for (int i = 0; i < springCoilSegments; i++)
        {
            ...
            Vector3 pathPoint = Vector3.Lerp(startPoint, endPoint, t);
